      ...
      {showCaptions && (
        <div className="morph-slider-caption" aria-live="polite">
          {items.map((item, i) => (
            <div
              key={i}
              className={`morph-slider-caption-content ${i === index ? 'is-active' : ''}`}
            ...
            >
              ...
              <div className="slider-tags">
                {item.tags?.map(tag => (
                  <span key={tag} className="slider-tag">{tag}</span>
                ))}
              </div>
              <a href={item.link} target="_blank" rel="noopener noreferrer" className="slider-btn">
                View Space &nbsp;↗
              </a>
            </div>
          ))}
